...
def mask_account_card(card: str | int) -> str:
    ...
    if len(card_split) == 2: # редполагается, что карта состоит только из типа карты и её номера
        ...
        new_number = get_mask_card_number(number) # импорт функции из masks.py
        card_connect = f"{card_split[0]} {card_split[1]} {new_number}" # соединение типа карты и номера
...
def get_date(number: str | int) -> str:
    ...
    format_date = f"{to_share[-1]}.{to_share[-2]}.{to_share[0]}" # преобразовываем элементы в нужнй формат
